...
--------------------------
 ...
 `jAVA` `SpringBoot` `JPA` `python`  `AWS` `NAVER Cloud` `Slack` `MariaDB` `MongoDB` `git` `MSA` `MVC` `HTML5` `CSS3` `RESTful` `JavaScript` `MySQL` `jQuery` `Docker` `GitHub Action`
...
관리자는 접속IP지정, 휴가승인, 사용지추가, 직원별 근태 확인 이 가능합니다 
