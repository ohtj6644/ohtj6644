...
<img src="https://capsule-render.vercel.app/api?type=waving&color=auto&height=200&section=header&text=Hello! I'm Glen&fontSize=90" />
--------------------------

 `도전을 즐기는 개발자`   Glen(오택주) 입니다!   현재 `기록물관리 솔루션 업체` 에서 근무중 입니다!  
 
 `jAVA` `SpringBoot` `JPA` `python`  `AWS` `NAVER Cloud` `Slack` `MySQL` `MariaDB` `MongoDB` `CUBRID` `git` `MSA` `MVC` `HTML5` `CSS3` `RESTful` `JavaScript` `MySQL` `jQuery` `Ajax` `Docker` `GitHub Action` `Bootstrap` `Linux` `mac` `Window`
